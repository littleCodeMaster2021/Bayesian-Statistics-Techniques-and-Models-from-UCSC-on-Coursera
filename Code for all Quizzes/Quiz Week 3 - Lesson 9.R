...
# Q4
X = model.matrix(mod_glm)[,-1] # -1 removes the column of the intercept(1 for all rows)
head(X)
X$ID <- seq.int(nrow(X))
plot(X[,3],X[,5])
...
#b0 + b[1]*Age[i] + b[2]*OMElow[i] + b[3]*Loud[i] + b[4]*Noiseincoherent[i]
age=60
OMElow = 0
Loud = 50
Noiseincoherent = 0
...
## The correct classification rate: 0.84 appears that the accurate cases (high probability of correct responses) are well captured by the model.
